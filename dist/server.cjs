...
var auth = (...requestedRoles) => {
  return catchAsync(async (req, res, next) => {
    ...
    if (!verifiedToken.success) {
      throw new AppError(import_http_status3.default.UNAUTHORIZED, verifiedToken.error);
    }
...
var validateRequest = (zodSchema) => {
  return catchAsync((req, res, next) => {
    ...
    if (!result.success) {
      ...
      throw new AppError(
        import_http_status4.default.BAD_REQUEST,
...
var UniversityCreateZodSchema = import_zod.default.object({
  name: import_zod.default.string().trim().min(2),
  shortName: import_zod.default.string().min(2).max(10)
});
var DepartmentCreateZodSchema = import_zod.default.object({
  name: import_zod.default.string().trim().min(2),
  code: import_zod.default.string().min(2).max(10),
  universityId: import_zod.default.string()
});
var ProgramCreateZodSchema = import_zod.default.object({
  name: import_zod.default.string().trim().min(2),
  code: import_zod.default.string().min(2).max(10),
  duration: import_zod.default.number().int().min(1).max(4, "Duration Maximum 4 Years"),
  totalCredits: import_zod.default.number().int().min(1).max(160),
  departmentId: import_zod.default.string("Not a string...")
});
var InstructorCreateZodSchema = import_zod.default.object({
  user: import_zod.default.object({
    name: import_zod.default.string().trim().min(2, "Name must be at least 2 characters long"),
    email: import_zod.default.email("Invalid email address").trim().toLowerCase(),
    password: import_zod.default.string().min(8, "Password must be at least 8 characters long").max(32, "Password cannot exceed 32 characters").regex(/[A-Z]/, "Password must contain at least one uppercase letter").regex(/[a-z]/, "Password must contain at least one lowercase letter").regex(/[0-9]/, "Password must contain at least one number").regex(
      /[^A-Za-z0-9]/,
      "Password must contain at least one special character"
    )
  }),
  instructor: import_zod.default.object({
    address: import_zod.default.string().trim().min(5, "Address must be at least 5 characters long").optional(),
    departmentId: import_zod.default.string().trim().min(2)
  })
});
var CourseCreateZodSchema = import_zod.default.object({
  title: import_zod.default.string().trim().min(2),
  code: import_zod.default.string().min(2).max(10),
  credit: import_zod.default.number("Not a Number...").int().min(1).max(3),
  semesterNo: import_zod.default.number("Not a Number...").int().min(1).max(12),
  programId: import_zod.default.string()
});
var SemesterCreateZodSchema = import_zod.default.object({
  name: import_zod.default.string().trim().min(2),
  year: import_zod.default.coerce.number("Year must be a number").int("Year must be an integer"),
  startDate: import_zod.default.coerce.date("Start date must be a valid date"),
  endDate: import_zod.default.coerce.date("End date must be a valid date")
});
var CourseOfferingCreateZodSchema = import_zod.default.object({
  courseId: import_zod.default.string().trim(),
  semesterId: import_zod.default.string().trim(),
  instructorId: import_zod.default.string().trim(),
  courseFee: import_zod.default.number("Not a Number...").int().min(1),
  totalSeat: import_zod.default.number("Not a Number...").int().min(1)
});
var AdminValidation = {
...
var AttendanceCreateZodSchema = import_zod2.default.object({
  courseRegistrationId: import_zod2.default.string().trim(),
  classDate: import_zod2.default.coerce.date("Start date must be a valid date"),
  status: import_zod2.default.string().trim()
});
...
var registerStudent = async (payload) => {
  ...
  if (isUserExisting) {
    throw new AppError(
      import_http_status8.default.CONFLICT,
      "User with this email already exists"
    );
  }
  const hashPassword = await import_bcryptjs2.default.hash(
    password,
    Number(config_default.bcrypt_salt_rounds)
  );
...
var verifyStudentEmail = async (payload) => {
  ...
  if (isUserExisting?.status === UserStatus.BLOCKED) {
    throw new AppError(import_http_status8.default.FORBIDDEN, "User is Blocked");
  }
  if (isUserExisting?.emailVerified) {
    throw new AppError(import_http_status8.default.CONFLICT, "Email Already Veryfied");
  }
  if (isUserExisting?.isDeleted || isUserExisting?.status === UserStatus.DELETED) {
    throw new AppError(import_http_status8.default.NOT_FOUND, "User is Deleted");
  }
  const otpKey = `student-registration-otp:${email}`;
  const redisOtp = await redisClient.get(otpKey);
  if (!redisOtp) {
    throw new AppError(import_http_status8.default.BAD_REQUEST, "Invalid OTP");
  }
  if (redisOtp !== otp) {
    throw new AppError(import_http_status8.default.BAD_REQUEST, "OTP Does Not Match");
  }
  await redisClient.del(otpKey);
  const studentRegistrationKey = `student-registration-data:${email}`;
  const redisStudentData = await redisClient.get(studentRegistrationKey);
  if (!redisStudentData) {
    throw new AppError(import_http_status8.default.NOT_FOUND, "Student Does not Exist");
  }
...
var loginUser = async (payload) => {
  ...
  if (!user) {
    throw new AppError(import_http_status8.default.NOT_FOUND, "User Not Found");
  }
  if (user.status === UserStatus.BLOCKED) {
    throw new AppError(import_http_status8.default.FORBIDDEN, "User Is Blocked");
  }
  ...
  const isPasswordMatched = await import_bcryptjs2.default.compare(
    password,
    user.password
  );
  if (!isPasswordMatched) {
    throw new AppError(import_http_status8.default.UNAUTHORIZED, "Invalid Credentials");
  }
  const jwtPayload = {
    userId: user.id,
    name: user.name,
...
var refreshToken = async (token) => {
  ...
  if (!verifiedRefreshToken.success || !verifiedRefreshToken.data) {
    throw new AppError(
      import_http_status8.default.UNAUTHORIZED,
  ...
  if (!user || user.isDeleted || user.status !== UserStatus.ACTIVE) {
    throw new AppError(import_http_status8.default.NOT_FOUND, "User is inactive or not found");
  }
  const jwtPayload = {
    userId: user.id,
    name: user.name,
...
var googleLogin = async (payload) => {
  ...
  if (!googleIdTokenPayload.email) {
    throw new AppError(import_http_status8.default.NOT_FOUND, "Google Email Not Found");
  }
  if (!googleIdTokenPayload.name) {
    throw new AppError(import_http_status8.default.NOT_FOUND, "Google Name Not Found");
  }
  ...
  if (!isStudentExistWithGoogleAuth) {
    ...
    if (ifPatientExistWithCredentials) {
      if (!ifPatientExistWithCredentials.emailVerified) {
        throw new AppError(import_http_status8.default.FORBIDDEN, "Email Not Verified");
      }
...
var refreshToken2 = catchAsync(async (req, res) => {
  if (!req.cookies.refreshToken) {
    throw new AppError(import_http_status9.default.NOT_FOUND, "Refresh Token is Missing");
  }
...
var StudentRegistrationZodSchema = import_zod3.default.object({
  name: import_zod3.default.string("Not a string....").min(3).max(10),
  email: import_zod3.default.email("Not Email...."),
  password: import_zod3.default.string().min(8, "Password must be at least 8 characters long").max(32, "Password cannot exceed 32 characters").regex(/[A-Z]/, "Password must contain at least one uppercase letter").regex(/[a-z]/, "Password must contain at least one lowercase letter").regex(/[0-9]/, "Password must contain at least one number").regex(
    /[^A-Za-z0-9]/,
    "Password must contain at least one special character"
  ),
  student: import_zod3.default.object({ address: import_zod3.default.string().optional() }).optional()
});
var StudentEmailVerifyZodSchema = import_zod3.default.object({
  email: import_zod3.default.email("Not Email...."),
  otp: import_zod3.default.string().length(6)
});
var ForgotPassowedZodShema = import_zod3.default.object({
  email: import_zod3.default.email("Not Email....")
});
var ResetPassowedZodShema = import_zod3.default.object({
  email: import_zod3.default.email("Not Email...."),
...
var updateCourseOffering = async (payload, id) => {
  ...
  if (!existingCourse) {
    throw new AppError(import_http_status10.default.NOT_FOUND, "Course Not Exists");
  }
  ...
  try {
    ...
    if ((bkashIdTokenTTL <= 600 || !bkashIdToken) && bkashRefreshToken && bkashRefreshTokenTTL > 600) {
      ...
      if (!refreshTokenresponse.ok) {
        throw new AppError(
          import_http_status12.default.BAD_GATEWAY,
    ...
    throw new AppError(import_http_status12.default.BAD_GATEWAY, error.message);
  }
};
...
var createCourseRegistration = async (payload, user) => {
  const transectionResult = await prisma.$transaction(async (tx) => {
    ...
    if (!student) {
      throw new AppError(import_http_status13.default.NOT_FOUND, "Student Profile Not Found");
    }
    ...
    if (!offeredCourse || offeredCourse.isDelete) {
      throw new AppError(import_http_status13.default.NOT_FOUND, "Course Not Found");
    }
    ...
    if (offeredCourse.availableSeat === 0) {
      throw new AppError(import_http_status13.default.BAD_REQUEST, "Seat Not Available");
    }
    if (!offeredCourse.courseFee) {
      throw new AppError(import_http_status13.default.BAD_REQUEST, "Course Fee Not Set Yet");
    }
    ...
    if (!bkashIdToken) {
      throw new AppError(import_http_status13.default.BAD_GATEWAY, "No Bkash Access Token Found");
    }
...
var courseRegistrationCallback = async (query) => {
  const transectionResult = await prisma.$transaction(
    async (tx) => {
      ...
      if (!paymentId) {
        throw new AppError(import_http_status13.default.BAD_REQUEST, "Payment Id Missing");
      }
      const status = query.status;
      if (!status) {
        throw new AppError(import_http_status13.default.BAD_REQUEST, "Payment Status Is Missing");
      }
      const bkashIdToken = await getBkashIdToken();
      if (!bkashIdToken) {
        throw new AppError(
      ...
      if (status === "success") {
        ...
        const pdfDocument = new import_pdfkit.default({
          size: "A4",
          margin: 50
        });
        const pdfChunks = [];
        ...
        drawLabelValue(
          "Payment Date",
          (0, import_date_fns.format)(executedPaymentResult.paymentExecuteTime, "dd-MMM-yyyy"),
          invoiceInfoY + 61
        );
...
var payCourseRegistration = async (payload, user) => {
  const transectionResult = await prisma.$transaction(async (tx) => {
    ...
    if (!existingRegistraion) {
      throw new AppError(import_http_status13.default.NOT_FOUND, "Course Registration Not Found");
    }
    ...
    const bkashIdToken = await getBkashIdToken();
    if (!bkashIdToken) {
      throw new AppError(import_http_status13.default.BAD_REQUEST, "No Bkash Access Token Found");
    }
    const bkashCreatePaymentResponse = await fetch(
      `${config_default.bkash_base_url}/tokenized/checkout/create`,
      {
...
var CourseRegistationValidationZodSchema = import_zod4.default.object({
  courseOfferingId: import_zod4.default.string()
});
var PayCourseRegistationValidationZodSchema = import_zod4.default.object({
  courseRegistrationId: import_zod4.default.string()
});
...
var createExam = async (payload) => {
  ...
  if (!offeredCourse) {
    throw new AppError(import_http_status15.default.NOT_FOUND, "Course Not Found");
  }
...
var ExamCreateZodSchema = import_zod5.default.object({
  courseOfferingId: import_zod5.default.string().trim(),
  type: import_zod5.default.string().trim(),
  toalMarks: import_zod5.default.number().min(1),
  examDate: import_zod5.default.coerce.date("Start date must be a valid date")
});
var ExamValidation = {
  ExamCreateZodSchema
};
...
var createResult = async (payload) => {
  ...
  if (!isCourseRedistrationExists) {
    throw new AppError(import_http_status17.default.NOT_FOUND, "Course Not Found");
  }
  ...
  if (!isExamExists) {
    throw new AppError(import_http_status17.default.NOT_FOUND, "Exam Not Found");
  }
  if (isCourseRedistrationExists.courseOfferingId !== isExamExists.courseOfferingId) {
    throw new AppError(import_http_status17.default.FORBIDDEN, "Offered Course Not Matched");
  }
...
var ResultCreateZodSchema = import_zod6.default.object({
  examId: import_zod6.default.string().trim(),
  courseRegistrationId: import_zod6.default.string().trim(),
  grade: import_zod6.default.string().trim(),
  marks: import_zod6.default.number().min(0),
  gradePoint: import_zod6.default.number().min(0)
});
...
var createTranscript = async (payload) => {
  ...
  if (!student) {
    throw new AppError(import_http_status20.default.NOT_FOUND, "Student Not Found");
  }
  const semester = await prisma.semester.findUnique({
    where: { id: semesterId }
  });
  if (!semester) {
    throw new AppError(import_http_status20.default.NOT_FOUND, "Semester Not Found");
  }
  ...
  if (isTranscriptExists) {
    throw new AppError(import_http_status20.default.BAD_REQUEST, "Transcript Already Generated");
  }
  ...
  if (courseRegistrations.length === 0) {
    throw new AppError(import_http_status20.default.BAD_REQUEST, "No Course Registration Found");
  }
  let totalCredit = 0;
  let earnedCredit = 0;
...
var TranscriptCreateZodSchema = import_zod7.default.object({
  studentId: import_zod7.default.string().trim(),
  semesterId: import_zod7.default.string().trim()
});
...
var getMe = async (user) => {
  ...
  if (!isUserExisting) {
    throw new AppError(import_http_status22.default.NOT_FOUND, "User Not Found");
  }
    ...
    (resolve, reject) => {
      cloudinary.uploader.upload_stream({ resource_type: "auto" }, async (error, result) => {
        ...
        if (!result) {
          return reject(
            new AppError(
              import_http_status22.default.BAD_GATEWAY,
...
var getMe2 = catchAsync(async (req, res) => {
  ...
  if (!user) {
    throw new AppError(
      import_http_status23.default.UNAUTHORIZED,
